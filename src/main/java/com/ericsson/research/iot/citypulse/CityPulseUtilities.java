 ...
 */
public class CityPulseUtilities {
    private SecureRandom randomID;


    public CityPulseUtilities() {
        randomID = new SecureRandom();
    }

    public String getNextRandomID() {
        return (new BigInteger(130, randomID).toString(32));
    }

    public String getHeaders(Vector<CityPulseObservation> observations) {
        StringBuilder headers = new StringBuilder();
        headers.append(hdr);

        Iterator<CityPulseObservation> iter = observations.iterator();
        int i = 0;

        headers.append("\n");
        while (iter.hasNext()) {
            CityPulseObservation currentObservation = iter.next();

            if (!currentObservation.getUOMNS().isEmpty()) {
                if (!headers.toString().contains(currentObservation.getUOMNS())) {
                    headers.append("@prefix unit" + i + ": <" + currentObservation.getUOMNS() + "> .\n");
                }
            }
            i++;
        }
    ...
    private String hdr = "@prefix ct: <http://www.insight-centre.org/citytraffic#> .\n"
